...
### ⚠️ قاعدة حرجة: هذا السؤال يستحق {max_score} درجة (ليس 10!)
**انتبه**: الدرجة القصوى هي {max_score} وليس 10. تأكد من أن درجتك بين 0 و {max_score}.

### قاعدة مهمة: حدد الإجابات فقط!
- قم بتقييم **إجابات الطالب فقط** - أي النص الذي كتبه الطالب كإجابة
- **لا تقم بتقييم** نص السؤال نفسه أو الأرقام المكتوبة مسبقاً (مثل س1-1، س1-2)
- إذا رأيت نص السؤال الأصلي، تجاهله - أنت تبحث عن إجابة الطالب فقط

### خطوة 1: عد الإجابات الفرعية في ورقة السؤال
احسب العدد الكلي للأسئلة الفرعية (T) في ورقة السؤال.
- مثال: إذا كان السؤال يحتوي على 12 سؤالاً فرعياً (1-1، 1-2، ... 1-12)، فـ T = 12

### خطوة 2: احسب نقاط كل سؤال فرعي
كل سؤال فرعي يستحق: {max_score} ÷ T نقاط
- مثال: إذا T = 10 والدرجة القصوى {max_score}:
  كل سؤال فرعي = {max_score} ÷ 10 = {max_score / 10:.1f} نقاط

### خطوة 3: عد الإجابات الصحيحة للطالب
احسب عدد الإجابات الصحيحة (C) التي كتبها الطالب:
- ✅ **صحيحة** = +1 إجابة صحيحة
- ⚠️ **جزئية** = +0.5 إجابة صحيحة
- ❌ **خاطئة** = 0

### خطوة 4: احسب الدرجة النهائية
الدرجة = (عدد الإجابات الصحيحة × نقاط كل سؤال فرعي)
الدرجة = C × ({max_score} ÷ T)

### 🔢 مثال عملي (لدرجة قصوى = {max_score}):
- ورقة السؤال تحتوي على 10 أسئلة فرعية (T = 10)
- نقاط كل سؤال فرعي = {max_score} ÷ 10 = {max_score / 10:.1f} نقاط
- الطالب أجاب: 2 صحيحة، 1 جزئية، 3 خاطئة
- C = 2 + 0.5 = 2.5 إجابة صحيحة
- الدرجة = 2.5 × {max_score / 10:.1f} = {2.5 * max_score / 10:.1f} من {max_score}
