...
### ⚠️ قاعدة مهمة جداً: حدد الإجابات فقط!
...
### خطوة 1: عد الإجابات المكتوبة
احسب عدد الإجابات التي كتبها الطالب فعلياً (N).
- إذا أجاب على 4 أسئلة فرعية، N = 4
- إذا لم يجب على بعض الأسئلة، لا تحسبها

### خطوة 2: احسب النقاط لكل إجابة
كل إجابة صحيحة تستحق ({max_score} ÷ N) نقاط تقريباً.
مثال: إذا أجاب على 5 أسئلة من أصل 10، وكانت الدرجة {max_score}:
- كل إجابة صحيحة = {max_score} ÷ 10 = {max_score / 10:.1f} نقاط (نقسم على العدد الكلي للأسئلة الفرعية)

### خطوة 3: طبق القواعد التالية بدقة:
- ✅ **إجابة صحيحة كاملة** = النقاط الكاملة للسؤال الفرعي
- ⚠️ **إجابة صحيحة جزئياً** = نصف النقاط
- ❌ **إجابة خاطئة** = 0 نقاط

### خطوة 4: اجمع النقاط
الدرجة النهائية = مجموع نقاط الإجابات الصحيحة فقط (يجب أن تكون بين 0 و {max_score})

### مثال حسابي (لدرجة قصوى = {max_score}):
- سؤال به 10 أجزاء فرعية، كل جزء = {max_score / 10:.1f} نقاط
- الطالب أجاب على 4 أجزاء: 2 صحيحة، 1 جزئي، 1 خطأ
- المجموع = ({max_score / 10:.1f} × 2) + ({max_score / 20:.2f}) + 0 = {(max_score / 10 * 2) + (max_score / 20):.1f} من {max_score}
